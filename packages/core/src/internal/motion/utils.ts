...
export function sizeDimensionKeyframes(animationKeyframes: AnimationKeyframes, hostEl: Element): AnimationKeyframes {
  ...
  return animationKeyframes.map((kf: Keyframe) => {
    if (kf?.hasOwnProperty('height') && isPrefixedBy(kf?.height?.toString() || '', 'from:')) { // eslint-disable-line
    ...
    if (kf?.hasOwnProperty('width') && isPrefixedBy(kf?.width?.toString() || '', 'from:')) { // eslint-disable-line
