...
describe('cds-grid-column', () => {
  ...
  it('should enable the resize action handle if resizable', async () => {
    ...
    expect(component.shadowRoot.querySelector('cds-action-resize').hasAttribute('readonly')).toBe(true);

    component.resizable = true;
    await componentIsStable(component);
    expect(component.shadowRoot.querySelector('cds-action-resize').hasAttribute('readonly')).toBe(false);
  });
});
