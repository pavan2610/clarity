...
export function dynamicGrid() {
  ...
  class DemoGridDynamic extends LitElement {
    @state() private data: DynamicGrid = {
      ...
      rows: data.map((i: any) => ({
        selected: false,
        cells: Object.keys(i).map((k: any) => ({ label: i[k] }))
      }))
    };

    render() {
      return html`
        <grid-dynamic .model=${this.data} @modelChange=${(e: any) => this.updateModel(e.detail)}></grid-dynamic>
      `;
    }
